...
const Edit = () => {
 ...
 useEffect(() => {
  async function getStudent() {
   try {
    const student = await axios.get(`http://localhost:3333/schema/${id}`)
 ...
 async function onFormSubmit(e) {
  e.preventDefault()
  try {
   await axios.put(`http://localhost:3333/schema/${id}`, student)
   history.push("/")
  } catch (error) {
   console.log("Something is Wrong");
  }
    ...
    <Grid item md={6} xs={12}>
     ...
     <form>
      <Grid container spacing={2}>
       ...
       <Grid item xs={12}>
        <TextField autoComplete="username" name="username" variant="outlined" required fullWidth id="username" label="Username" value={student.username} onChange={e => onTextFieldChange(e)} />
       </Grid>
       <Grid item xs={12}>
